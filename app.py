...
def fetch_saudi_data_proxy(ticker):
    """نظام جلب هجين ومحصن بثلاثة روابط بديلة لمنع الحظر السحابي تماماً"""
    urls = [
        f"https://yahoo.com{ticker}.SR?range=5d&interval=15m",
        f"https://yahoo.com{ticker}.SR?range=5d&interval=15m"
    ]
...
with st.spinner("🔄 جاري الطواف اللحظي وجلب بيانات الشركات القيادية من السحاب..."):
    for ticker in saudi_leaders:
        ...
        # محاكاة خطوط الطلب المرنة لتفادي تداخل البيانات خارج وقت الجلسة
        df['Current_Demand'] = df['Low'].rolling(5).min()
        df['Current_Supply'] = df['High'].rolling(5).max()
        df['Current_Demand'], df['Current_Supply'] = df['Current_Demand'].ffill(), df['Current_Supply'].ffill()
        
        stock_dfs[ticker] = df
        last = df.iloc[-1]
        
        is_golden = "سهم ذهبي مستعد 🌟" if last['Close'] <= (last['Current_Demand'] * 1.05) and last['RSI'] < 70 else "انتظار صامت ⏸️"
...
# 🎛️ 4. توزيع الواجهة الرسومية بعد تخطي قيود الحظر
if market_data_list:
    df_market = pd.DataFrame(market_data_list)
    col_list, col_chart, col_calc = st.columns([1.2, 2, 1.2])
    
    with col_list:
        st.subheader("🎯 قائمة الرصد")
        selected_ticker = st.selectbox("اختر السهم للمعاينة وعرض المستويات الفنية:", df_market['رمز السهم'].tolist())
    ...
    st.warning("⏸️ خوادم البيانات في وضع الصيانة اللحظية الآن؛ ثوانٍ معدودة وسيفتح جرس افتتاح السوق وتنبثق المنصة والجدول تلقائياً أمام عينك حياً.")
